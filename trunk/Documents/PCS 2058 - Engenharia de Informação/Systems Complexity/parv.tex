...
Quando se fala em arquitetura de rede, é natural que se pense na Internet. Isso faz com que seja imprescindível uma análise da complexibilidade não de se usá-la, mas intrínseca à ela, ou seja, o quão complexa tal rede é e quais fatores alimentam tal condi\c{c}ão. Para isso, basta analisar a evolu\c{c}ão --- desde sua cria\c{c}ão --- do número de computadores conectados. No entanto, existem outros atributos de Redes de Computadores que devem ser analisados, como topologia de rede, tipo de conexão (computadores, conexões por dispositivos móveis, etc..) e outros tipos de redes, como \textit{Intranets}, Redes Pessoas (PAN's --- \textit{Personal Area Networks}).\par
Este estudo tem como foco principal a Internet. Cogitou-se utilizar o número de usuários. Contudo, este é limitado pela população, e mesmo que a Internet tenha penetração de 100\%, outras formas de acesso (como dispositivos móveis, por exemplo) podem existir e contribuir para o aumento de complexidade. o que explica a necessidade de outra métrica. O número de \emph{hosts}, embora não seja a métrica desejada, é proporcional ao crescimento da complexidade.
...
\subsection{Arquitetura de Software}
	As métricas mais facilmente aplicáveis para a medida de complexidad de sistemas de software são as métricas de tamanho e complexidade estrutural. Algumas das mais utilizadas são: número de linhas de código, número de métodos, número de componentes, complexidade ciclomática de métodos.

%	As tabelas listadas abaixo ilustram os resultados de um estudo feito por \cite{vasa03}. Neste estudo, foram avaliados os aspectos de complexidade estrutural e complexidade de tamanho de alguns programas, utilizando para isso as métricas de complexidade ciclomática e número de linhas de código. Os programas foram avaliados ao longo de algumas versões de desenvolvimento, como indicado na tabela

%\begin{table}
%\begin{tabular}{|c|c}
%\hline
%Versão & Número de linhas de código não-comentadas


%\end{tabular}

%	\tiny \caption{Programas avaliados em \cite{vasa03}}
%	\label{table:programas avaliados}
%\end{table}
.

\section{Arquitetura de Rede}
Duas variáveis já mencionadas anteriormente serão abordadas: número de usuários e \textit{hosts}. As figuras \ref{fig:user_growth} e \ref{fig:hosts_growth} mostram, respectivamente, a evolução do número de usuários e do número de \emph{hosts} ao longo dos anos desde a criação da Internet. O comportamento de ambos gráficos reflete um crescimento exponencial.\par

\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.75,bb=0 0 600 797]{images/users.png}
	\tiny \caption{Evoluc\c{c}ão do número de usuários da Internet, \cite{iwsWEB}}
	\label{fig:user_growth}
\end{figure}
\begin{figure}[ht!]
	\centering
	\includegraphics[scale=0.5,bb=0 0 680 407]{images/hosts.png}
